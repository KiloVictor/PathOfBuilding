...
void POBWindow::SetDrawLayer(int layer) { SetDrawLayer(layer, 0); }
...
void POBWindow::DrawColor(const float col[4]) {
  ...
  /*QColor qcol(drawColor[0] * 255, drawColor[1] * 255, drawColor[2] * 255,
  drawColor[3] * 255);
  painter->setPen(qcol);
...
static int l_SetCallback(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "SetCallback() argument 1: expected string, got %t", 1);
  lua_pushvalue(L, 1);
  if (n >= 2) {
    pobwindow->LAssert(
        L, lua_isfunction(L, 2) || lua_isnil(L, 2),
        "SetCallback() argument 2: expected function or nil, got %t", 2);
    lua_pushvalue(L, 2);
  } else {
    lua_pushnil(L);
  }
...
static int l_GetCallback(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "GetCallback() argument 1: expected string, got %t", 1);
  lua_pushvalue(L, 1);
...
static int l_SetMainObject(lua_State *L) {
  ...
  if (n >= 1) {
    pobwindow->LAssert(
        L, lua_istable(L, 1) || lua_isnil(L, 1),
        "SetMainObject() argument 1: expected table or nil, got %t", 1);
    lua_pushvalue(L, 1);
  } else {
    lua_pushnil(L);
  }
...
static int l_NewImageHandle(lua_State *L) {
  imgHandle_s *imgHandle =
      (imgHandle_s *)lua_newuserdata(L, sizeof(imgHandle_s));
  imgHandle->hnd = NULL;
  imgHandle->img = NULL;
...
static imgHandle_s *GetImgHandle(lua_State *L, const char *method,
                                 bool loaded) {
  pobwindow->LAssert(L, pobwindow->IsUserData(L, 1, "uiimghandlemeta"),
                     "imgHandle:%s() must be used on an image handle", method);
  imgHandle_s *imgHandle = (imgHandle_s *)lua_touserdata(L, 1);
  lua_remove(L, 1);
  if (loaded) {
    // pobwindow->LAssert(L, imgHandle->hnd != NULL, "imgHandle:%s(): image
    // handle has no image loaded", method);
  }
  return imgHandle;
}
...
static int l_imgHandleLoad(lua_State *L) {
  ...
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 1,
                     "Usage: imgHandle:Load(fileName[, flag1[, flag2...]])");
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "imgHandle:Load() argument 1: expected string, got %t", 1);
  ...
  for (int f = 2; f <= n; f++) {
    ...
    if (!strcmp(flag, "ASYNC")) {
      ...
      pobwindow->LAssert(L, 0, "imgHandle:Load(): unrecognised flag '%s'",
                         flag);
    }
  }
...
static int l_imgHandleSetLoadingPriority(lua_State *L) {
  ...
  pobwindow->LAssert(
      L, lua_isnumber(L, 1),
      "imgHandle:SetLoadingPriority() argument 1: expected number, got %t", 1);
  // pobwindow->renderer->SetShaderLoadingPriority(imgHandle->hnd,
...
static int l_SetClearColor(lua_State *L) {
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 3,
                     "Usage: SetClearColor(red, green, blue[, alpha])");
  float color[4];
  for (int i = 1; i <= 3; i++) {
    pobwindow->LAssert(L, lua_isnumber(L, i),
                       "SetClearColor() argument %d: expected number, got %t",
                       i, i);
  ...
  if (n >= 4 && !lua_isnil(L, 4)) {
    pobwindow->LAssert(
        L, lua_isnumber(L, 4),
        "SetClearColor() argument 4: expected number or nil, got %t", 4);
...
static int l_SetDrawLayer(lua_State *L) {
  ...
  pobwindow->LAssert(
      L, lua_isnumber(L, 1) || lua_isnil(L, 1),
      "SetDrawLayer() argument 1: expected number or nil, got %t", 1);
  if (n >= 2) {
    pobwindow->LAssert(L, lua_isnumber(L, 2),
                       "SetDrawLayer() argument 2: expected number, got %t", 2);
  }
  if (lua_isnil(L, 1)) {
    pobwindow->LAssert(L, n >= 2,
                       "SetDrawLayer(): must provide subLayer if layer is nil");
...
static int l_SetViewport(lua_State *L) {
  ...
  if (n) {
    ...
    for (int i = 1; i <= 4; i++) {
      pobwindow->LAssert(L, lua_isnumber(L, i),
                         "SetViewport() argument %d: expected number, got %t",
                         i, i);
    }
    pobwindow->AppendCmd(std::shared_ptr<Cmd>(
        new ViewportCmd((int)lua_tointeger(L, 1), (int)lua_tointeger(L, 2),
                        (int)lua_tointeger(L, 3), (int)lua_tointeger(L, 4))));
  } else {
    QSize s(pobwindow->painter->window().size());
    pobwindow->AppendCmd(
        std::shared_ptr<Cmd>(new ViewportCmd(0, 0, s.width(), s.height())));
  }
  return 0;
}

static int l_SetDrawColor(lua_State *L) {
  pobwindow->LAssert(L, pobwindow->painter != NULL,
                     "SetDrawColor() called outside of OnFrame");
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 1, "Usage: SetDrawColor(red, green, blue[, "
                                "alpha]) or SetDrawColor(escapeStr)");
  float color[4];
  if (lua_type(L, 1) == LUA_TSTRING) {
    pobwindow->LAssert(
        L, IsColorEscape(lua_tostring(L, 1)),
        "SetDrawColor() argument 1: invalid color escape sequence");
    ReadColorEscape(lua_tostring(L, 1), color);
    color[3] = 1.0;
  } else {
    pobwindow->LAssert(L, n >= 3, "Usage: SetDrawColor(red, green, blue[, "
                                  "alpha]) or SetDrawColor(escapeStr)");
    for (int i = 1; i <= 3; i++) {
      pobwindow->LAssert(L, lua_isnumber(L, i),
                         "SetDrawColor() argument %d: expected number, got %t",
                         i, i);
      color[i - 1] = (float)lua_tonumber(L, i);
    }
    if (n >= 4 && !lua_isnil(L, 4)) {
      pobwindow->LAssert(
          L, lua_isnumber(L, 4),
          "SetDrawColor() argument 4: expected number or nil, got %t", 4);
      color[3] = (float)lua_tonumber(L, 4);
    } else {
      color[3] = 1.0;
    }
...
static int l_DrawImage(lua_State *L) {
  pobwindow->LAssert(L, pobwindow->painter != NULL,
                     "DrawImage() called outside of OnFrame");
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 5, "Usage: DrawImage({imgHandle|nil}, left, top, "
                                "width, height[, tcLeft, tcTop, tcRight, "
                                "tcBottom])");
  pobwindow->LAssert(
      L, lua_isnil(L, 1) || pobwindow->IsUserData(L, 1, "uiimghandlemeta"),
      "DrawImage() argument 1: expected image handle or nil, got %t", 1);
  ...
  if (!lua_isnil(L, 1)) {
    ...
    if (imgHandle->hnd == NULL) {
      ...
      if (!imgHandle->hnd->isCreated()) {
        // std::cout << "BROKEN TEXTURE " <<
        // imgHandle->img->text("fname").toStdString() << std::endl;
        delete imgHandle->hnd;
        imgHandle->hnd = pobwindow->white;
      }
    }
    pobwindow->LAssert(L, imgHandle->hnd != NULL,
                       "DrawImage(): image handle has no image loaded");
    hnd = imgHandle->hnd;
  }
  float arg[8];
  if (n > 5) {
    ...
    for (int i = 2; i <= 9; i++) {
      pobwindow->LAssert(L, lua_isnumber(L, i),
                         "DrawImage() argument %d: expected number, got %t", i,
                         i);
      arg[i - 2] = (float)lua_tonumber(L, i);
    }
    pobwindow->AppendCmd(std::shared_ptr<Cmd>(new DrawImageCmd(
        hnd, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7])));
  } else {
    for (int i = 2; i <= 5; i++) {
      pobwindow->LAssert(L, lua_isnumber(L, i),
                         "DrawImage() argument %d: expected number, got %t", i,
    ...
    pobwindow->AppendCmd(std::shared_ptr<Cmd>(
        new DrawImageCmd(hnd, arg[0], arg[1], arg[2], arg[3])));
  }
  return 0;
}
...
static int l_DrawImageQuad(lua_State *L) {
  pobwindow->LAssert(L, pobwindow->painter != NULL,
                     "DrawImageQuad() called outside of OnFrame");
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 9, "Usage: DrawImageQuad({imgHandle|nil}, x1, y1, "
                                "x2, y2, x3, y3, x4, y4[, s1, t1, s2, t2, s3, "
                                "t3, s4, t4])");
  pobwindow->LAssert(
      L, lua_isnil(L, 1) || pobwindow->IsUserData(L, 1, "uiimghandlemeta"),
      "DrawImageQuad() argument 1: expected image handle or nil, got %t", 1);
  QOpenGLTexture *hnd = NULL;
  if (!lua_isnil(L, 1)) {
    imgHandle_s *imgHandle = (imgHandle_s *)lua_touserdata(L, 1);
    if (imgHandle->hnd == NULL) {
    ...
    pobwindow->LAssert(L, imgHandle->hnd != NULL,
                       "DrawImageQuad(): image handle has no image loaded");
    hnd = imgHandle->hnd;
  }
  float arg[16];
  if (n > 9) {
    ...
    for (int i = 2; i <= 17; i++) {
      pobwindow->LAssert(L, lua_isnumber(L, i),
                         "DrawImageQuad() argument %d: expected number, got %t",
                         i, i);
      arg[i - 2] = (float)lua_tonumber(L, i);
    }
    pobwindow->AppendCmd(std::shared_ptr<Cmd>(new DrawImageQuadCmd(
        hnd, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7],
...
DrawStringCmd::DrawStringCmd(float X, float Y, int Align, int Size, int Font,
                             const char *Text)
    : text(Text) {
  if (text[0] == '^') {
  ...
  int count = 0;
  for (QRegularExpressionMatchIterator i =
           QRegularExpression("(\\^x.{6})|(\\^\\d)").globalMatch(text);
       i.hasNext(); i.next()) {
    count += 1;
  }
  if (count > 1) {
...
static int l_DrawString(lua_State *L) {
  pobwindow->LAssert(L, pobwindow->painter != NULL,
                     "DrawString() called outside of OnFrame");
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 6,
                     "Usage: DrawString(left, top, align, height, font, text)");
  pobwindow->LAssert(L, lua_isnumber(L, 1),
                     "DrawString() argument 1: expected number, got %t", 1);
  pobwindow->LAssert(L, lua_isnumber(L, 2),
                     "DrawString() argument 2: expected number, got %t", 2);
  pobwindow->LAssert(L, lua_isstring(L, 3) || lua_isnil(L, 3),
                     "DrawString() argument 3: expected string or nil, got %t",
                     3);
  pobwindow->LAssert(L, lua_isnumber(L, 4),
                     "DrawString() argument 4: expected number, got %t", 4);
  pobwindow->LAssert(L, lua_isstring(L, 5),
                     "DrawString() argument 5: expected string, got %t", 5);
  pobwindow->LAssert(L, lua_isstring(L, 6),
                     "DrawString() argument 6: expected string, got %t", 6);
  static const char *alignMap[6] = {"LEFT",     "CENTER",  "RIGHT",
                                    "CENTER_X", "RIGHT_X", NULL};
  static const char *fontMap[4] = {"FIXED", "VAR", "VAR BOLD", NULL};
  // std::cout << "Drawstring: " << lua_tostring(L, 6) << " | align: " <<
  // lua_tostring(L, 3) << std::endl;
  pobwindow->AppendCmd(std::shared_ptr<Cmd>(new DrawStringCmd(
      (float)lua_tonumber(L, 1), (float)lua_tonumber(L, 2),
      luaL_checkoption(L, 3, "LEFT", alignMap), (int)lua_tointeger(L, 4),
      luaL_checkoption(L, 5, "FIXED", fontMap), lua_tostring(L, 6))));
  return 0;
}
...
static int l_DrawStringWidth(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isnumber(L, 1),
                     "DrawStringWidth() argument 1: expected number, got %t",
                     1);
  pobwindow->LAssert(L, lua_isstring(L, 2),
                     "DrawStringWidth() argument 2: expected string, got %t",
                     2);
  pobwindow->LAssert(L, lua_isstring(L, 3),
                     "DrawStringWidth() argument 3: expected string, got %t",
                     3);
  static const char *fontMap[4] = {"FIXED", "VAR", "VAR BOLD", NULL};
  ...
  QFont font(pobwindow->fontName,
             fontsize + pobwindow->fontFudge); // fontname, fontsize);
  QFontMetrics fm(font);
  lua_pushinteger(L, fm.width(text));
  return 1;
  //    lua_pushinteger(L, pobwindow->DrawStringWidth((int)lua_tointeger(L, 1),
  //    luaL_checkoption(L, 2, "FIXED", fontMap), lua_tostring(L, 3)));
  return 1;
}
...
static int l_DrawStringCursorIndex(lua_State *L) {
  ...
  pobwindow->LAssert(
      L, n >= 5,
      "Usage: DrawStringCursorIndex(height, font, text, cursorX, cursorY)");
  pobwindow->LAssert(
      L, lua_isnumber(L, 1),
      "DrawStringCursorIndex() argument 1: expected number, got %t", 1);
  pobwindow->LAssert(
      L, lua_isstring(L, 2),
      "DrawStringCursorIndex() argument 2: expected string, got %t", 2);
  pobwindow->LAssert(
      L, lua_isstring(L, 3),
      "DrawStringCursorIndex() argument 3: expected string, got %t", 3);
  pobwindow->LAssert(
      L, lua_isnumber(L, 4),
      "DrawStringCursorIndex() argument 4: expected number, got %t", 4);
  pobwindow->LAssert(
      L, lua_isnumber(L, 5),
      "DrawStringCursorIndex() argument 5: expected number, got %t", 5);
  static const char *fontMap[4] = {"FIXED", "VAR", "VAR BOLD", NULL};
  //    lua_pushinteger(L,
  //    pobwindow->DrawStringCursorIndex((int)lua_tointeger(L, 1),
  //    luaL_checkoption(L, 2, "FIXED", fontMap), lua_tostring(L, 3),
...
static int l_StripEscapes(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "StripEscapes() argument 1: expected string, got %t", 1);
...
static int l_NewFileSearch(lua_State *L) {
  int n = lua_gettop(L);
  pobwindow->LAssert(L, n >= 1,
                     "Usage: NewFileSearch(spec[, findDirectories])");
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "NewFileSearch() argument 1: expected string, got %t", 1);
  ...
  QFileInfoList fil = dir.entryInfoList(
      filters, QDir::NoDotAndDotDot | (dirOnly ? QDir::Dirs : QDir::Files));
  if (fil.isEmpty()) {
    return 0;
  }

  searchHandle_s *handle =
      (searchHandle_s *)lua_newuserdata(L, sizeof(searchHandle_s));
...
static QFileInfoList *GetSearchHandle(lua_State *L, const char *method,
                                      bool valid) {
  pobwindow->LAssert(L, pobwindow->IsUserData(L, 1, "uisearchhandlemeta"),
                     "searchHandle:%s() must be used on a search handle",
                     method);
  searchHandle_s *searchHandle = (searchHandle_s *)lua_touserdata(L, 1);
  lua_remove(L, 1);
  if (valid) {
    pobwindow->LAssert(L, !searchHandle->fil->isEmpty(),
                       "searchHandle:%s(): search handle is no longer valid "
                       "(ran out of files to find)",
                       method);
  }
...
static int l_SetWindowTitle(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "SetWindowTitle() argument 1: expected string, got %t", 1);
...
static int l_GetCursorPos(lua_State *L) {
  ...
  lua_pushinteger(L, floor(pos.x() * pobwindow->devicePixelRatio()));
  lua_pushinteger(L, floor(pos.y() * pobwindow->devicePixelRatio()));
  return 2;
}
...
static int l_SetCursorPos(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isnumber(L, 1),
                     "SetCursorPos() argument 1: expected number, got %t", 1);
  pobwindow->LAssert(L, lua_isnumber(L, 2),
                     "SetCursorPos() argument 2: expected number, got %t", 2);
  // pobwindow->sys->video->SetRelativeCursor((int)lua_tointeger(L, 1),
...
static int l_IsKeyDown(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "IsKeyDown() argument 1: expected string, got %t", 1);
  size_t len;
...
static int l_Copy(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "Copy() argument 1: expected string, got %t", 1);
...
static int l_Deflate(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "Deflate() argument 1: expected string, got %t", 1);
  z_stream_s z;
  z.zalloc = NULL;
  z.zfree = NULL;
  deflateInit(&z, 9);
...
static int l_Inflate(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "Inflate() argument 1: expected string, got %t", 1);
  size_t inLen;
  Byte *in = (Byte *)lua_tolstring(L, 1, &inLen);
  int outSz = inLen * 4;
  Byte *out = new Byte[outSz];
...
static int l_MakeDir(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "MakeDir() argument 1: expected string, got %t", 1);
...
static int l_RemoveDir(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "l_RemoveDir() argument 1: expected string, got %t", 1);
  QDir d;
...
static int l_SetWorkDir(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "SetWorkDir() argument 1: expected string, got %t", 1);
...
static int l_LaunchSubScript(lua_State *L) {
  ...
  pobwindow->LAssert(
      L, n >= 3,
      "Usage: LaunchSubScript(scriptText, funcList, subList[, ...])");
  for (int i = 1; i <= 3; i++) {
    pobwindow->LAssert(L, lua_isstring(L, i),
                       "LaunchSubScript() argument %d: expected string, got %t",
                       i, i);
  }
  for (int i = 4; i <= n; i++) {
    pobwindow->LAssert(L,
                       lua_isnil(L, i) || lua_isboolean(L, i) ||
                           lua_isnumber(L, i) || lua_isstring(L, i),
                       "LaunchSubScript() argument %d: only nil, boolean, "
                       "number and string types can be passed to sub script",
                       i);
  }
...
static int l_AbortSubScript(lua_State *L) {
  ...
  /*
    ...
    pobwindow->LAssert(L, lua_islightuserdata(L, 1), "AbortSubScript() argument
    1: expected subscript ID, got %t", 1);
    notdword slot = (notdword)lua_touserdata(L, 1);
    pobwindow->LAssert(L, slot < pobwindow->subScriptSize &&
    pobwindow->subScriptList[slot], "AbortSubScript() argument 1: invalid
    subscript ID");
    pobwindow->LAssert(L, pobwindow->subScriptList[slot]->IsRunning(),
...
static int l_IsSubScriptRunning(lua_State *L) {
  ...
  /*
    ...
    pobwindow->LAssert(L, lua_islightuserdata(L, 1), "IsSubScriptRunning()
    argument 1: expected subscript ID, got %t", 1);
    notdword slot = (notdword)lua_touserdata(L, 1);
    pobwindow->LAssert(L, slot < pobwindow->subScriptSize &&
    pobwindow->subScriptList[slot], "IsSubScriptRunning() argument 1: invalid
    subscript ID");
...
static int l_LoadModule(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "LoadModule() argument 1: expected string, got %t", 1);
  QString fileName(lua_tostring(L, 1));
  ...
  pobwindow->LAssert(L, err == 0, "LoadModule() error loading '%s':\n%s",
                     fileName.toStdString().c_str(), lua_tostring(L, -1));
...
static int l_PLoadModule(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "PLoadModule() argument 1: expected string, got %t", 1);
  QString fileName(lua_tostring(L, 1));
  if (!fileName.endsWith(".lua")) {
    fileName = fileName + ".lua";
  }
...
static int l_PCall(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isfunction(L, 1),
                     "PCall() argument 1: expected function, got %t", 1);
  //    lua_getfield(L, LUA_REGISTRYINDEX, "traceback");
  //    lua_insert(L, 1); // Insert traceback function at start of stack
...
static int l_ConPrintf(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "ConPrintf() argument 1: expected string, got %t", 1);
  lua_pushvalue(L, lua_upvalueindex(1)); // string.format
  lua_insert(L, 1);
  lua_call(L, n, 1);
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "ConPrintf() error: string.format returned non-string");
  ...
  while (lua_next(L, index)) {
    ...
    if (lua_type(L, -1) == LUA_TTABLE) {
      ...
      if (expand) {
        ...
        std::cout << "table: " << lua_topointer(L, -1) << " {" << std::endl;
...
static int l_ConPrintTable(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_istable(L, 1),
                     "ConPrintTable() argument 1: expected table, got %t", 1);
...
static int l_ConExecute(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "ConExecute() argument 1: expected string, got %t", 1);
...
static int l_print(lua_State *L) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    pobwindow->LAssert(L, s != NULL,
                       "print() error: tostring returned non-string");
    if (i > 1)
      std::cout << " ";
    std::cout << s;
    lua_pop(L, 1); // Pop result of tostring
...
static int l_SpawnProcess(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "SpawnProcess() argument 1: expected string, got %t", 1);
  // FIXME
...
static int l_OpenURL(lua_State *L) {
  ...
  pobwindow->LAssert(L, lua_isstring(L, 1),
                     "OpenURL() argument 1: expected string, got %t", 1);
  // FIXME
...
static int l_Exit(lua_State *L) {
  ...
  if (n >= 1 && !lua_isnil(L, 1)) {
    pobwindow->LAssert(L, lua_isstring(L, 1),
                       "Exit() argument 1: expected string or nil, got %t", 1);
